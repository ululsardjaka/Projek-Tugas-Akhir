...
KEYBOARD_PILIHAN = [['Cuaca Sekarang', 'Cuaca Harian', 'Gempa'], ['Tentang', 'Keluar']]
...
def pilihan(update: Update, context: CallbackContext) -> int:
    ...
    elif user_input == 'gempa':
        reply = gempa(update, context)
    else:
...
def query_search(update: Update, context: CallbackContext) -> int:
    ...
    try:
        result = soup.findAll('div', {'class': 'locations-list'})[0].findAll('a', href=True)
        reply = ''
        lokasi = []
...
def gempa(update: Update, context: CallbackContext) -> int:
    try:
        ...
        '⬇️ Kedalaman	:	{}
        '📳 Dirasakan	:	{}
        '🌊 Potensi		:	{}'''.format (tanggal, jam, wilayah, coordinates, lintang, bujur, magnitude, kedalaman, potensi, dirasakan)
        update.message.reply_text(
                reply,
                reply_markup=ReplyKeyboardMarkup(KEYBOARD_PILIHAN),
        )     
